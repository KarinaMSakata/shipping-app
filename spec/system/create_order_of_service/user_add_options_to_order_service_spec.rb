...
describe 'Usuário adiciona Modalidade e Veículo na ordem de serviço' do
  ...
  it 'e peso da carga não pode ser superior a carga máxima do veículo' do
    user = User.create!(name: 'Karina', email: 'karina@sistemadefrete.com.br', password:'password', role: :user)
    ...
    fiorino = Vehicle.create!(sort: 'Carro', brand: 'Fiat', model: 'Fiorino', identification:'ABC1D44', year_manufacture:'2019', max_load: 250, status: :in_operation)

    saveiro = Vehicle.create!(sort: 'Carro', brand: 'Volkswagen', model: 'Saveiro', identification:'KLJ1D84', year_manufacture:'2019', max_load: 300, status: :in_operation)

    os = CreateOrderOfService.create!(output_address: 'Av. das Pitangueiras, 100', output_city: 'São Paulo', output_state: 'SP',
                                      product_code: 'LG-LCD-49-5502-P', height: 50, width: 90, depth: 5, cargo_weight: 200,
                                      receiver_address: 'Rua das Flores, 20', receiver_city: 'Osasco', receiver_state: 'SP', 
                                      receiver_name: 'João de Almeida Santos', receiver_cpf: '37688849020', receiver_birth: '23/01/1990',
                                      total_distance: 41)
    ...
    #Assert
    expect(page).to have_content 'Veículo: Carro Fiat | Fiorino'
    expect(page).not_to have_content 'Veículo: Carro Fiat | Doblo'
    expect(page).not_to have_content 'Veículo: Carro Mercedes | Sprinter'
    expect(page).not_to have_content 'Veículo: Carro Volkswagen | Saveiro'
  end

  
  it 'e link para adicionar fica indisponível' do
    user = User.create!(name: 'Karina', email: 'karina@sistemadefrete.com.br', password:'password', role: :user)
    ...
    #Assert
    expect(page).not_to have_link 'Adicionar Modalidade e Veículo'
    expect(page).to have_link 'Alterar Modalidade'
  end

  it 'e inicia a ordem de serviço' do
     #Arrange
     user = User.create!(name: 'Karina', email: 'karina@sistemadefrete.com.br', password:'password', role: :user)
     ...
     expect(page).not_to have_link 'Alterar Modalidade'
  end

  it 'e veículo escolhido fica indisponível para outras ordens' do
    user = User.create!(name: 'Karina', email: 'karina@sistemadefrete.com.br', password:'password', role: :user)
    ...
    #Assert
    expect(page).to have_content 'Carro Fiat | Doblo [Em manutenção]'
  end
end